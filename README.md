...
## Metode
Vi træner og sammenligner 5 modeller:
1. **Linear Regression** — simpel baseline model
2. **Deep Neural Network (DNN)** — neural netværk med 3 skjulte lag
3. **Decision Tree** — enkelt beslutnings-træ
4. **XGBoost** — tree-based ensemble model
5. **XGBoost (Optuna)** — XGBoost med automatisk hyperparameter tuning

Bygget med Anthropic Claude API + XGBoost (Optuna) model.
...
| Decision Tree | Scikit-learn (DecisionTreeRegressor) |
...
| DNN | 109 kr. | 0.70 |
| Decision Tree | 118 kr. | 0.65 |
| XGBoost | 103 kr. | 0.73 |
| **XGBoost (Optuna)** | **85 kr.** | **0.82** |

XGBoost med Optuna tuning er den bedste model — lavest fejl og højest R².
